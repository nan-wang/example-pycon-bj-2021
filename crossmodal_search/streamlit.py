...
APP_NAME = 'Cross Modal Search'
APP_DESCRIPTION = 'Looking For Images From Captions and vice versa'
...
def request_and_display(query_input, top_k):
    ...
    display_photos(matches, search_type)


# Display match photos:
def display_photos(matches, search_type):
    st.markdown(
        f"""
    ...
    for i in range(row):
        ...
        with col1:
            if i * 2 < cnt:
                ...
                st.markdown(f"#### No.{i * 2 + 1}: Score  {score:.3f}\n ")
                st.image(uri)
        with col2:
            if i * 2 + 1 < cnt:
                # search_type 1:search with text  0:search with image
                ...
                st.markdown(f"####  No.{i * 2 + 2}:Score  {score:.3f}\n ")
                st.image(uri)
...
def main():
    ...
    input_choice = st.sidebar.selectbox(
        ...
        [{'name': 'search_by_text', 'display': 'input text and search for photos'},
         {'name': 'search_by_image', 'display': 'upload photo and search for description'}],
    ...
    elif input_choice == 'search_by_image':
    ...
    if clicked:
        if (not query_text) and (not query_image_file):
            ...
            request_and_display(query_text or query_image_file.name, top_k)
            if query_image_file:
